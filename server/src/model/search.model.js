...
Search.findAll = async (userId, query, result) => {
  ...
  const [data] = await promiseDb.query(
    ` SELECT * FROM ( ` +
      `   SELECT 'playlist' AS type, p.id , p.title, u.name as author, NULL AS name, p.user_id, p.image_path , p.public, fpc.count, p.created_at as created_at` +
      `   FROM playlists AS p ` +
      ...
      ` UNION ` +
      `   SELECT 'song' AS type, s.id, s.title as title, u.name as author, null as name, s.user_id, s.image_path, s.public as public, fsc.count ,s.created_at as created_at ` +
      `   FROM songs AS s ` +
      ...
      ` UNION ` +
      `   SELECT 'artist' AS type, u.id, NUll as title, NUll as author, u.name, u.id as user_id, u.image_path, NULL as public, fl.count ,Null as created_at ` +
      `   FROM users AS u ` +
...
Search.findSongs = async (userId, query, result) => {
  ...
  const [data] = await promiseDb.query(
    ` SELECT 'song' AS type, s.id , s.title, u.name as author, NULL AS name , s.user_id, s.image_path , s.public, fsc.count ,s.created_at as created_at` +
      ` FROM songs AS s` +
      ` LEFT JOIN users as u on u.id = s.user_id ` +
      ` LEFT JOIN favourite_songs_count as fsc on fsc.song_id = s.id ` +
      ` WHERE ((s.public = 1 ) OR (s.user_id = '${userId}')) AND s.is_deleted = 0 ` +
